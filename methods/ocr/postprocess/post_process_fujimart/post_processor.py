...
class PostProcessorFUJIMART(BaseModule):
    def __init__(self, common_config, model_config):
        super(PostProcessorFUJIMART, self).__init__(common_config, model_config)
    ...
    def predict(self, request_id, info):
        ...
        for key in info['raw_result'].keys():
            if key == 'products':
                ...
                if key == 'date':
                    result['result']['time'] = self.post_processors['time'].predict(None, None, None, info['raw_result'][key], info['charset_list'])[-1]
        return result
