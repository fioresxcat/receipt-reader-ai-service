...
class Base_LM:
    ...
    # def get_word_cands(self, cands, k=10):
    #     word_cands = []
    #     for w_cand in cands:
    #         best_cands = [([], 0)]
    #         for char_cands in w_cand:
    #             new_best_cands = []
    #             for i in range(len(char_cands)):
    #                 for j in range(len(best_cands)):
    #                     prob = best_cands[j][1] + math.log(char_cands[i][1] + 1e-30)
    #                     new_best_cands.append((best_cands[j][0] + [char_cands[i][0]], prob))
    #             new_best_cands.sort(key = lambda x: -x[1])
    #             best_cands = new_best_cands[:k]

    #         word_cand = []
    #         for list_char, prob in best_cands:
    #             word = ''
