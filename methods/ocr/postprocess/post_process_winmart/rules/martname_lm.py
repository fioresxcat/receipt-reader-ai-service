...
class Martname_LM(Base_LM):
    def __init__(self, dict_path, max_words):
        ...
        with open(dict_path, 'r', encoding='utf-8') as f:
            for row in f:
                raw, label = row[:-1].split('\t')
                self.values.add(raw.lower())
                self.map[raw.lower()] = label
    ...
    def check_rule(self, word_cands):
        ...
        if self.max_words == None:
            ...
            for value in self.values:
                score = Levenshtein.distance(string, unidecode.unidecode(value.lower()))
                if score < min_score:
                    final_value = self.map[value]
                    min_score = score
            if min_score > 20 + len(string)*0.1*self.threshold:
