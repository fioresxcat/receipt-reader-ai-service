...
class ProductName_LM(Base_LM):
    def __init__(self, dict_path, max_words):
        ...
        self.sign_groups = []
        self.sign_groups.append(set(['a', 'á', 'à', 'ả', 'ã', 'ạ', 'ă', 'ằ', 'ắ', 'ẳ', 'ẵ', 'ặ', 'â', 'ấ', 'ầ', 'ẩ', 'ẫ', 'ậ', '4']))
        self.sign_groups.append(set(['o', 'ò', 'ó', 'ỏ', 'õ', 'ọ', 'ô', 'ồ', 'ố', 'ổ', 'ỗ', 'ộ', 'ơ', 'ớ', 'ờ', 'ở', 'ỡ', 'ợ']))
        self.sign_groups.append(set(['u', 'ù', 'ú', 'ủ', 'ũ', 'ụ', 'ư', 'ừ', 'ứ', 'ử', 'ữ', 'ự']))
        self.sign_groups.append(set(['e', 'è', 'é', 'ẻ', 'ẽ', 'ẹ', 'ê', 'ề', 'ế', 'ể', 'ễ', 'ệ']))
        self.sign_groups.append(set(['i', 'ì', 'í', 'ỉ', 'ĩ', 'ị']))
        self.sign_groups.append(set(['d', 'đ']))


    def unsign(self, a, b):
        ok = False
        for group in self.sign_groups:
    ...
    def find_similar_child(self, node, word):
        ...
        for child_name in node.children.keys():
            ...
            if score <= int(self.similar_threshold * len(word) / 2):
                similar_nodes.append((node.children[child_name], score))
        similar_nodes.sort(key = lambda x: x[1])
        similar_nodes = [s[0] for s in similar_nodes]
    ...
    def predict(self, key, index, sub_key, char_cands, charset_list):
        ...
        raw_result = ' '.join(raw_result).lower().strip().split(' ')
