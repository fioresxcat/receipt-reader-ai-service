...
class PostProcessorPEPPERLUNCH(BaseModule):
    def __init__(self, common_config, model_config):
        super(PostProcessorPEPPERLUNCH, self).__init__(common_config, model_config)
    ...
    def predict(self, request_id, info):
        ...
        # add product_quantity, product_unit_price to product with positive product_total_money
            ...
            for product in info['result']['products']:
                if 'product_total_money' in product.keys():
                    if re.search('^\-', product['product_total_money']) is not None:
                        ...
                        if ('product_unit_price' not in product.keys() or product['product_unit_price'] == '') and 'product_quantity' in product.keys() and product['product_quantity'] == '1':
