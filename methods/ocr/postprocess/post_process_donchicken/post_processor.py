...
class PostProcessorDONCHICKEN(BaseModule):
    def __init__(self, common_config, model_config):
        super(PostProcessorDONCHICKEN, self).__init__(common_config, model_config)
    ...
    def predict(self, request_id, info):
        ...
        # # missing receipt id
        # if 'pos_id' in result['result'].keys() and ('receipt_id' not in result['result'].keys() or result['result']['receipt_id'] == ''):
        #     result['result']['receipt_id'] = self.post_processors['receipt_id'].predict(None, None, None, info['raw_result']['pos_id'], info['charset_list'])[-1]
