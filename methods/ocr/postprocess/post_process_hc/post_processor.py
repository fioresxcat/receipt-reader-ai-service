...
class PostProcessorHC(BaseModule):
    def __init__(self, common_config, model_config):
        super(PostProcessorHC, self).__init__(common_config, model_config)
    ...
    def predict(self, request_id, info):
        ...
        for key in info['raw_result'].keys():
            if key == 'products':
                ...
                if key == 'time' and len(info['raw_result'][key]) == 0 and len(info['raw_result']['date']) != 0:
                     result['result'][key] = self.post_processors[key].predict(None, None, None, info['raw_result']['date'], info['charset_list'])[-1]
                if key == 'date' and len(info['raw_result'][key]) == 0 and len(info['raw_result']['time']) != 0:
                     result['result'][key] = self.post_processors[key].predict(None, None, None, info['raw_result']['time'], info['charset_list'])[-1]
        return result
