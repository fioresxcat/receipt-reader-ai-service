...
class PostProcessorOKONO(BaseModule):
    def __init__(self, common_config, model_config):
        super(PostProcessorOKONO, self).__init__(common_config, model_config)
    ...
    def predict(self, request_id, info):
        ...
        # postprocess date in receipt id
        if 'date' in result['result'].keys() and result['result']['date'] == '' and 'receipt_id' in result['result'].keys() and result['result']['receipt_id'] != '':
            result['result']['date'] = self.post_processors['date'].predict(None, None, None, info['raw_result']['receipt_id'], info['charset_list'])[-1]
        return result
