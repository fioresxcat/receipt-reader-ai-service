...
class Base_LM:
    ...
    def get_sentence_cands(self, word_cands, k=10):
        best_cands = [(0., [], 1.)]
        for w_cand in word_cands:
            new_best_cans = []
            for word, prob in w_cand:
                for i in range(len(best_cands)):
                    new_word = best_cands[i][1] + [word]
                    new_log_prob = best_cands[i][0] + prob
                    new_min_prob = min(best_cands[i][2], prob)
                    if len(new_best_cans) < k:
                        heapq.heappush(new_best_cans, (new_log_prob, new_word, new_min_prob))
                    else:
                        heapq.heappushpop(new_best_cans, (new_log_prob, new_word, new_min_prob))
    ...
    def get_word_cands_num(self, cands, charset_list, max_word=100, max_char=100, k=10, charset='0123456789', max_char_cand=20):
        ...
        for w_cand in cands[:max_word]:
            ...
            for raw_char_cands in w_cand[:max_char]:
                ...
                new_best_cans = []
                found_char = False
                for i in range(len(char_cands)):
                    if found_char and i >= max_char_cand:
                        break
            ...
            for prob, list_char in best_cands:
                ...
                word_cand.append((word, math.exp(prob/max_char)))
