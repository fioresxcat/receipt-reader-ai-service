...
class PostProcessorLOTTECINEMA(BaseModule):
    def __init__(self, common_config, model_config):
        super(PostProcessorLOTTECINEMA, self).__init__(common_config, model_config)
    ...
    def init_processor(self, config):
        ...
        elif config['type'] == 'Product_Name_LM':
            return Product_Name_LM()
    ...
    def predict(self, request_id, info):
        ...
        if 'products' in info['result'].keys():
            ...
            for product in info['result']['products']:
                ...
                elif 'product_unit_price' in product.keys() and product['product_unit_price'] != '' and 'product_name' in product.keys() and product['product_name'] != '':
                    if re.search('u22|adult|special disabled', product['product_name'].lower()) is not None:
                        if product['product_unit_price'].replace(',', '').isdecimal() and product['product_quantity'].replace(',', '').isdecimal():
                            temp_money = product['product_unit_price'].replace(',', '')
                            temp_money = int(temp_money) * int(product['product_quantity'].replace(',', ''))
                            temp_money = str(temp_money)
                            new_temp_money = ''
                            while len(temp_money) > 0:
                                new_temp_money = temp_money[-3:] + ',' + new_temp_money
                                temp_money = temp_money[:-3]
                            product['product_total_money'] = new_temp_money[:-1]
                        new_products.append(product)
            info['result']['products'] = new_products
        if 'receipt_id' in result['result'].keys():
            result['result']['barcode'] = result['result']['receipt_id']
        return result
