...
class PostProcessorMEGA(BaseModule):
    def __init__(self, common_config, model_config):
        super(PostProcessorMEGA, self).__init__(common_config, model_config)
    ...
    def init_processor(self, config):
        if config['type'] == 'Value_LM':
            ...
            return Greed_LM(config['max_words'])
        ...
        elif config['type'] == 'Mart_id_LM':
            return Mart_id_LM(config['max_words'])
        elif config['type'] == 'ProductDiscountCode_LM':
            return ProductDiscountCode_LM(config['max_words'])
    ...
    def predict(self, request_id, info):
        ...
        if 'pos_id' in result['result'].keys() and 'receipt_id' in result['result'].keys():
            result['result']['receipt_id'] = 'ISR' + result['result']['pos_id'] + result['result']['receipt_id'][-9:]
        if 'discounts' not in result['result'].keys():
